// findpending-mini.cjs — 找出当前 pendingInteraction 的会话及其父级关系
...
const proc = spawn('C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe', [
  ...
  '--user-data-dir=~/AppData\\Local\\Temp\\wt-chrome-findpending',
  'about:blank',
], { stdio: 'ignore' });
...
(async () => {
  ...
  const step = await evaluate(`(function(){
    ...
    var byId = snap.byId || {};
    var out = { pendings: [], subs: {} };
    Object.keys(byId).forEach(function(k){
      var e = byId[k];
      if (e.pendingInteraction != null) {
        out.pendings.push({ id: k.slice(0,14), title: (e.displayTitle||'').slice(0,24), parentId: e.parentId ? e.parentId.slice(0,14) : null, pending: JSON.stringify(e.pendingInteraction), running: e.running });
      }
    });
    ...
    Object.keys(subs).forEach(function(pid){
      ...
      out.subs[pid.slice(0,14)] = (arr||[]).map(function(c){ return { id: (c.sessionId||c.id||'').slice(0,14), kind: c.kind, activity: c.activity }; });
    });
    return JSON.stringify(out);
  })()`);
  console.log('FINDPENDING:', JSON.stringify(step).slice(0, 2400));
