...
/**
 * dsh-tianshu 服务端：健康路由 + 工作区内容窗的数据路由。
...
function setupTerminal(webServer: any, ctx: any) {
  ...
  ctx.logger?.info?.('[dsh-tianshu] term deps: ws=' + (wsMod ? 'ok' : 'MISSING') + ' node-pty=' + (ptyMod ? 'ok' : 'MISSING'))
  if (!wsMod || !ptyMod) {
    ctx.logger?.warn('[dsh-tianshu] 终端路由未注册：ws/node-pty 不可用')
    return
  }
  ...
  }), 'dsh-tianshu: terminal upgrade')
}

export function apply(ctx: Context) {
  const webServer = (ctx as any).webServer
  if (!webServer) {
    ctx.logger?.warn('[dsh-tianshu] ctx.webServer 不可用（headless profile？），跳过服务端路由')
    return
  }
  ...
  webServer.register({
    ...
    handler: (_req: any, res: any) => {
      json(res, 200, { plugin: 'dsh-tianshu', version: PLUGIN_VERSION, ok: true })
    },
  })
