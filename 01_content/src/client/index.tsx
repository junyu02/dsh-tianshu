...
/**
 * dsh-tianshu 客户端（v2）：侧边栏底部「工作台」区块。
...
const KNOWLEDGE_PACK = [
  ...
  '- dsh-tianshu 是 DeepSeek Harness 的自建容器插件（工作台：侧边栏里的项目应用抽屉；不了解的细节可直接向用户提问）。',
...
function buildWindowTaskText(projectId: string, projectName: string, windowLabel: string, requirement: string, folder: string | null, mode: 'new' | 'send'): string {
  ...
  const lines = [
    '【工作台自定义窗口任务】',
    '以下内容由 dsh-tianshu（工作台）插件' + (mode === 'new' ? '自动发送' : '发送') + '：用户想把「' + projectName + '」项目中的「' + win + '」窗口打造成他想要的内容。',
    '1. dsh-tianshu 是侧边栏底部的自建「工作台」插件（官方文档没有它的说明，不了解之处可直接向用户提问）。',
    '2. 用户需求：' + requirement,
    '3. ' + folderLine,
...
function buildCustomLayoutPrompt(req: string): string {
  return [
    '【为 dsh-tianshu（工作台）插件增加一个新的布局预设】',
    '',
    '背景：dsh-tianshu 是 DeepSeek Harness 的自建容器插件（本机仓库 dsh-tianshu；不了解的细节可直接向用户提问）。',
...
export function apply(ctx: any) {
  ...
  ctx.effect(() => {
    ...
    style.setAttribute('data-dsh-plugin', 'dsh-tianshu')
  ...
  syncIds()
  ctx.effect(() => disposeSubscribe, 'dsh-tianshu: project registry watch')
  ...
  if (sessionsList && typeof sessionsList.getSnapshot === 'function') {
    ...
    ctx.effect(() => disposeScope, 'dsh-tianshu: session scope watch')
  }

  // 分栏工作区浮层（M1 通用引擎，shell.overlay 座位）
  ctx.slots.inject('shell.overlay', () => ctx.slots.register({
    name: 'shell.overlay',
    id: 'dsh-tianshu-split',
    order: 100,
  }, SplitWorkspace), 'dsh-tianshu: split workspace overlay')
  ...
  }, WorktableSection), 'dsh-tianshu: worktable section')
}
